...
const reviewSchema = new mongoose.Schema(
  {
    review: {
      type: String,
      required: [true, 'Tour require review'],
    },
    rating: {
      type: Number,
      min: 1,
    ...
    tour: {
      ...
      ref: 'Tour',
      required: [true, 'review must have a tours'],
    },
    user: {
      type: mongoose.Schema.ObjectId,
      ref: 'User',
...
//createdAt static methods
reviewSchema.statics.calcRatingsAvg = async function (tourId) {
  //using aggregation pipeline, this points to current model
  const stats = await this.aggregate([
    //stage one is matching
    {
      $match: { tour: tourId },
    },
    //grouping stage next
